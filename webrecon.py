...
# Safe-by-default TLD extraction:
# - Uses the bundled Public Suffix List snapshot (no network refresh)
# - Disables cache writes (works in read-only/containerized environments)
_SAFE_TLD_EXTRACT = tldextract.TLDExtract(suffix_list_urls=None, cache_dir=None)
...
class URLUtils:
    ...
    def get_domain(url):
        ...
        extracted = _SAFE_TLD_EXTRACT(url)
...
class Config:
    def __init__(self):
        ...
        # Security / Safety Configuration (hardened defaults)
        self.VERIFY_TLS = True  # Disable only with explicit --insecure
        self.OPEN_BROWSER = False  # Disable side-effectful browser automation by default
        self.ENABLE_PDF_SEARCH = False  # Disable Google dork auto-search by default
        self.SCOPE = "same-domain"  # same-domain | same-host | any
        self.BLOCK_PRIVATE_NETWORKS = True  # Block localhost/private/link-local/metadata ranges by default
        self.IGNORE_ROBOTS = False  # Respect robots.txt by default
        self.MAX_RESPONSE_BYTES = 5_000_000  # Cap per-response body read (bytes, decompressed)
        self.DEBUG = False
    ...
    def download_image(self, image_url, page_url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Save image (enforce max size even when Content-Length is missing)
                bytes_written = 0
                with open(raw_filepath, 'wb') as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        ...
                        if bytes_written > self.config.MAX_IMAGE_SIZE:
                            raise ValueError(
                                f"Image exceeded max size ({self.config.MAX_IMAGE_SIZE} bytes)"
                            )
                        f.write(chunk)
                ...
                ColorOutput.image(
                    f"Downloaded: {safe_filename} ({self._format_file_size(file_size)}) from {page_url}"
                )
            ...
            try:
                if 'raw_filepath' in locals() and raw_filepath and os.path.exists(raw_filepath):
    ...
    def _extract_image_metadata(self, image_path, image_url, page_url):
        ...
        try:
            ...
            if getattr(self.config, "DEBUG", False):
                ColorOutput.info(f"Image metadata extraction error: {str(e)[:200]}")
...
class DataExtractor:
    ...
    def extract_subdomains(self, urls):
        ...
        for url in urls:
            try:
                parsed = urllib.parse.urlparse(url)
                extracted = _SAFE_TLD_EXTRACT(parsed.netloc)
...
class DNSDumpsterAutomation:
    ...
    def open_in_browser(self, domain, open_browser=False):
        ...
        if not open_browser:
            ColorOutput.info("Browser automation disabled; open the URL manually if needed.")
            return
        import webbrowser
...
class WHOISLookup:
    ...
    def open_in_browser(self, domain, open_browser=False):
        ...
        if not open_browser:
            ColorOutput.info("Browser automation disabled; WHOIS Service URLs:")
            for url in whois_services:
                ColorOutput.info(f"   {url}")
            return
...
class WaybackMachine:
    ...
    def get_historical_data(self, domain):
        ...
        try:
            wayback_url = f"https://web.archive.org/cdx/search/cdx?url={domain}/*&output=json&collapse=urlkey&limit=10"
            ...
            if getattr(self.config, "DEBUG", False):
                ColorOutput.info(f"Wayback lookup error: {str(e)[:200]}")
    ...
    def harvest_emails(self, domain, crawled_urls):
        ...
        for url in sample_urls:
            try:
                ...
                if getattr(self.config, "DEBUG", False):
                    ColorOutput.info(f"Email harvest error for {url}: {str(e)[:120]}")
                continue
...
class PDFExtractor:
    def open_pdf_search(self, domain, open_browser=False):
        ...
        ColorOutput.info("PDF SEARCH")
        if not open_browser:
            ColorOutput.info("Browser automation disabled; manual PDF Search URLs:")
            for search in searches:
                google_url = f"https://www.google.com/search?q={urllib.parse.quote(search)}"
                ColorOutput.info(f"Google: {google_url}")
            return
...
class TechnologyDetector:
    ...
    def detect_technologies(self, url):
        ...
        try:
            ...
            if getattr(self.config, "DEBUG", False):
                ColorOutput.info(f"Technology detection error: {str(e)[:200]}")
...
class WebCrawler:
    def __init__(self, config, proxy=None):
        ...
        self._host_safety_cache = {}

    def _create_session(self):
        session = requests.Session()
        session.headers.update({'User-Agent': self.config.USER_AGENT})
        session.verify = bool(getattr(self.config, "VERIFY_TLS", True))
    ...
    def _init_scope(self, start_url):
        self.base_url = start_url
        self.base_domain = URLUtils.get_domain(start_url)
        parsed = urllib.parse.urlparse(start_url)
        self.base_host = (parsed.hostname or "").lower()

    def _in_scope(self, url):
        scope = getattr(self.config, "SCOPE", "same-domain")
        if scope == "any":
            return True

        parsed = urllib.parse.urlparse(url)
        host = (parsed.hostname or "").lower()
        if not host or not self.base_host or not self.base_domain:
            return False
        ...
        # same-domain
        try:
            return URLUtils.get_domain(url) == self.base_domain
        except Exception:
            return False

    def _hostname_is_private_or_reserved(self, hostname):
        """Return True if hostname resolves to a non-global IP (private/loopback/link-local/etc)."""
        if not hostname:
            return True

        key = hostname.lower()
        if key in self._host_safety_cache:
            return self._host_safety_cache[key]

        # Obvious local hostnames
        if key in {"localhost"} or key.endswith(".local"):
            self._host_safety_cache[key] = True
            return True
        ...
        try:
            ...
            self._host_safety_cache[key] = unsafe
            return unsafe
        except ValueError:
            pass

        # If we're routing via an explicit proxy, DNS resolution may be unavailable on isolated
        # internal networks (and the proxy should enforce private-network blocks anyway).
        if self.proxy:
            self._host_safety_cache[key] = False
            return False

        # Resolve hostname to IPs and evaluate
        try:
            ...
            # Conservative default: treat unknown/unresolvable hosts as unsafe when blocking private networks
            self._host_safety_cache[key] = True
            return True

    def _is_allowed_url(self, url):
        if not URLUtils.is_valid_url(url) or URLUtils.should_skip_url(url):
            return False

        if not self._in_scope(url):
            return False

        if getattr(self.config, "BLOCK_PRIVATE_NETWORKS", True):
            hostname = urllib.parse.urlparse(url).hostname or ""
            if self._hostname_is_private_or_reserved(hostname):
                return False

        if self.robots_parser and not getattr(self.config, "IGNORE_ROBOTS", False):
            try:
                if not self.robots_parser.can_fetch(self.config.USER_AGENT, url):
                    return False
            except Exception:
                # If robots parsing fails, default-allow
                pass

        return True

    def _read_response_limited(self, response):
        """Read response body up to MAX_RESPONSE_BYTES, decoding to text."""
        max_bytes = int(getattr(self.config, "MAX_RESPONSE_BYTES", 5_000_000))
        ...
        for chunk in response.iter_content(chunk_size=8192):
            ...
            size += len(chunk)
            if size > max_bytes:
                raise ValueError("Response exceeded max size")
            chunks.append(chunk)
    ...
    def fetch_url(self, url):
        """Fetch URL content without following out-of-policy redirects.

        Returns: (content|None, headers|None, final_url|None, redirects:list[dict])
        """
        redirects = []
        ...
        try:
            ...
            for _ in range(max_redirects + 1):
                ...
                # Handle redirects manually so we can enforce policy before the next request.
                if 300 <= response.status_code < 400 and response.headers.get("location"):
                    location = response.headers.get("location", "")
                    next_url = urllib.parse.urljoin(current_url, location)

                    allowed = self._is_allowed_url(next_url)
                    redirects.append(
                        {
                            "from": current_url,
                            "to": next_url,
                    ...
                    if not allowed:
                        return None, dict(response.headers), current_url, redirects
                ...
                return content, dict(response.headers), current_url, redirects

            # Too many redirects
            return None, None, url, redirects

        except Exception as e:
            if getattr(self.config, "DEBUG", False):
                ColorOutput.info(f"Fetch error for {url}: {str(e)[:200]}")
            return None, None, None, redirects
    ...
    def discover_urls(self, start_url):
        ...
        robots_sitemaps = self._parse_robots_txt(robots_url)

        sitemap_urls = self._parse_sitemap(start_url, extra_sitemaps=robots_sitemaps)
    ...
    def _parse_robots_txt(self, robots_url):
        """Parse robots.txt for crawler policy + sitemap hints."""
        sitemaps = set()

        if getattr(self.config, "IGNORE_ROBOTS", False):
            return sitemaps
        ...
        try:
            ...
            # Extract Sitemap: hints
            for line in lines:
                if line.lower().startswith("sitemap:"):
                    sitemap_url = line.split(":", 1)[1].strip()
                    if sitemap_url and URLUtils.is_valid_url(sitemap_url):
                        sitemaps.add(sitemap_url)

            if sitemaps:
                ColorOutput.success(f"Found {len(sitemaps)} sitemap URLs in robots.txt")
        except Exception as e:
            if getattr(self.config, "DEBUG", False):
                ColorOutput.info(f"robots.txt parse error: {str(e)[:200]}")

        return sitemaps

    def _parse_sitemap(self, base_url, extra_sitemaps=None):
        ...
        if extra_sitemaps:
            for url in extra_sitemaps:
                if URLUtils.is_valid_url(url):
                    sitemap_urls.append(url)

        for sitemap_url in sitemap_urls:
            try:
                if not self._is_allowed_url(sitemap_url):
                    continue
                ...
                if response.status_code == 200:
                    ...
                    for loc in soup.find_all('loc'):
                        url = loc.text.strip()
                        if URLUtils.is_valid_url(url) and self._is_allowed_url(url):
                            urls.add(url)
                    if urls:
    ...
    def _extract_urls_from_page(self, url):
        ...
        urls = set()
        content, headers, final_url, redirects = self.fetch_url(url)
        ...
        if content:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if URLUtils.is_valid_url(full_url) and self._is_allowed_url(full_url):
                    urls.add(full_url)
    ...
    def crawl(self, start_url, max_pages=None, max_depth=None, download_images=True):
        ...
        # Initialize scope policy for this crawl
        self._init_scope(start_url)
        ...
        if not self._is_allowed_url(start_url):
            ColorOutput.info("Start URL blocked by policy (scope/robots/private network).")
            ColorOutput.info("Try: --scope any, --ignore-robots, and/or --allow-private (only if authorized).")
        ...
        while queue and len(self.visited_urls) < max_pages:
            ...
            if url in self.visited_urls or depth > max_depth or not self._is_allowed_url(url):
                continue
            ...
            content, headers, final_url, redirects = self.fetch_url(url)
            if redirects:
                all_findings['redirects'].extend(redirects)

            # Treat redirects as a "hit" even if we didn't fetch body content.
            if content is not None or redirects:
                self.visited_urls.add(final_url or url)
                all_findings['crawled_links'].add(final_url or url)

                # Only parse/extract when we actually have response body content.
...
class WebReconPro:
    ...
    def run_advanced_reconnaissance(self, start_url, max_pages=None, max_depth=None, output_file=None,
        ...
        open_browser = bool(getattr(self.config, "OPEN_BROWSER", False))
        enable_pdf_search = bool(getattr(self.config, "ENABLE_PDF_SEARCH", False))
        ...
        try:
            ...
            if enable_dnsdumpster:
                ...
                dnsdumpster.open_in_browser(domain, open_browser=open_browser)
            ...
            if enable_whois:
                ...
                whois_lookup.open_in_browser(domain, open_browser=open_browser)

            if enable_wayback:
                ColorOutput.info("Wayback Machine historical analysis...")
                wayback = WaybackMachine(self.config, proxy=proxy)
                self.results['wayback_data'] = wayback.get_historical_data(domain)
                wayback.open_in_browser(domain, open_browser=open_browser)

            ColorOutput.info("Performing advanced email harvesting...")
            email_harvester = AdvancedEmailHarvester(self.config, proxy=proxy)
            ...
            if enable_pdf_search:
                pdf_extractor = PDFExtractor()
                pdf_extractor.open_pdf_search(domain, open_browser=open_browser)

            ColorOutput.info("Performing technology detection...")
            tech_detector = TechnologyDetector(self.config, proxy=proxy)
            ...
            if enable_builtwith:
                ...
                builtwith.open_in_browser(domain, open_browser=open_browser)
...
{Fore.GREEN}Safety / Hardening:{Style.RESET_ALL}
  --scope MODE           URL scope: same-domain | same-host | any (default: same-domain)
  --ignore-robots        Ignore robots.txt (default: respect)
  --allow-private        Allow localhost/private/link-local targets (default: blocked)
  --insecure             Disable TLS verification (default: verify)
  --open-browser         Enable opening external URLs in your browser (default: off)
  --pdf-search           Enable PDF Google dork search (default: off)
  --suppress-warnings    Suppress warnings (default: show)
...
{Fore.GREEN}Examples:{Style.RESET_ALL}
  python3 webrecon.py https://example.com --max-pages 25 --max-depth 2
  python3 webrecon.py https://example.com --proxy socks5://127.0.0.1:9050 --output results.json
  python3 webrecon.py https://example.com --no-images
  python3 webrecon.py https://example.com --pdf-search --open-browser
    """)
...
{Style.RESET_ALL}
    ...
    parser.add_argument('--scope', choices=['same-domain', 'same-host', 'any'], default='same-domain',
                        help='URL scope policy (default: same-domain)')
    parser.add_argument('--ignore-robots', action='store_true', help='Ignore robots.txt (default: respect)')
    parser.add_argument('--allow-private', action='store_true',
                        help='Allow localhost/private/link-local/metadata targets (default: blocked)')
    parser.add_argument('--insecure', action='store_true', help='Disable TLS verification (default: verify)')
    parser.add_argument('--open-browser', action='store_true',
                        help='Allow opening external URLs in a browser (default: disabled)')
    parser.add_argument('--pdf-search', action='store_true',
                        help='Enable PDF Google dork search (default: disabled)')
    ...
    config.OPEN_BROWSER = bool(args.open_browser)
    config.ENABLE_PDF_SEARCH = bool(args.pdf_search)
    config.SCOPE = args.scope
    config.IGNORE_ROBOTS = bool(args.ignore_robots)
    config.BLOCK_PRIVATE_NETWORKS = not bool(args.allow_private)
